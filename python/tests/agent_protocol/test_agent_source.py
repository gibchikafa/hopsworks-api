...
import pytest
from hopsworks_agent_eval import agent_source as src
...
class TestCloning:
    ...
    def test_the_users_git_providers_are_put_where_git_reads_them(
        ...
        written = src.configure_git_credentials(
            home=str(tmp_path),
            environ={
                "GIT_CREDENTIALS": "https://alice:tok@github.com https://bob:t2@gitlab.example.com",
                "GIT_NAME": "Alice A",
                "GIT_EMAIL": "alice@example.com",
            },
            run=lambda command, **kwargs: calls.append(
                (command, kwargs.get("env", {}).get("HOME"))
            ),
        )
        assert written == 2
        ...
        assert credentials == [
            ...
            "https://bob:t2@gitlab.example.com",
        ]
    ...
    def test_the_clone_url_never_carries_a_token(self, tmp_path, monkeypatch):
        ...
        ).read_text().strip() == "https://alice:secret@github.com"
